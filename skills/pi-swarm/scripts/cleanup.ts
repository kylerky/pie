...
import { computePaths, removeIfExists, ShellError, SocketError } from "./lib/common.ts";
...
const program = Effect.gen(function* () {
  ...
  if (liveOrphans.length > 0) {
    ...
    yield* Console.log(
      "  Use kill.ts to terminate them if they are unwanted.",
    );
  } else {
