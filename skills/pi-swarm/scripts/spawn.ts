...
const program = Effect.gen(function* () {
	...
	if (!parsed.name || !parsed.prompt.trim()) {
		...
		yield* Console.error("  [--no-skills | -ns] [--skill <path>]...");
		yield* Console.error("  [--no-context-files | -nc]");
		yield* Console.error("  [--provider <name> --model <model>]");
		yield* Console.error("  <name> <initial-prompt>");
