...
Detect the terminal's color scheme (dark or light) using a chain of detection methods, and automatically switch Pi's theme to match. Supports terminals that implement DEC private mode 2031 (Kitty, WezTerm), OSC 11 background color queries (most terminals), and the COLORFGBG environment variable as a final fallback.
